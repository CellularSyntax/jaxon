...
vagus-nerve anatomies (Section~\ref{sec:results-duke}).  The
fibre-model generalisation matters: the same optimisation pipeline
applies to myelinated and unmyelinated fibres with no algorithmic
change.  Because jaxon simulates the full fibre population rather than
a per-fascicle centroid, it furthermore quantifies --- for the first
time --- the \emph{deployment penalty} incurred when a configuration
optimised on sparsely-sampled fibres is run on the full nerve, and
shows that this penalty is governed by the number of fibres per target
fascicle and thereby mediates a cross-species gap in transferable
selectivity (Section~\ref{sec:results-sparse}).
...
established simulator of extracellular peripheral-fibre stimulation.
...
end-to-end gradients; gradient-based optimisation over design
