"""Kilohertz frequency block — Sundt C-fiber.

Analogous to khz_block.py but for the Sundt (2015) unmyelinated C-fiber model.

Setup
-----
* Sundt fiber, D=1.0 µm, N_NODES=25
...
Outputs (outputs/khz_block_sundt/)
------------------------------------
  data_khz_block_sundt.json
  fig_khz_block_sundt_traces.png

Run from project root:
    python experiments_v2/khz_block_sundt.py
"""
...
OUT = ensure_dir(ROOT / "outputs" / "khz_block_sundt")
...
AP_DETECT_LOC = 0.9
AMPLITUDES    = [-1.0, -5.0, -15.0, -20.0]   # mA: ctrl → kHz-induced → partial block → block
...
def main():
    ...
    fig.suptitle(f"kHz block — Sundt D={DIAMETER} µm", fontsize=11)
    path = OUT / "fig_khz_block_sundt_traces.png"
    ...
    save_json({
        ...
        "results":       results,
    }, OUT / "data_khz_block_sundt.json")
    print(f"  -> {OUT / 'data_khz_block_sundt.json'}")
    print("\n=== Done ===")
