"""Verify fix across all 8 pulse shapes for MRG D=5.7, 2 PWs each."""
...
def mfn(Vm, state, dt):
    ...
    return jnp.where(is_node,g_node,g_pas_us), jnp.where(is_node,i_node,g_pas_us*(Vm-V_REST)), (M2,H2,MP2,S2)

Ve_j = jnp.asarray(Ve_unit, dtype=jnp.float64)
@jax.jit
def run(amp_mA, pulse_shape):
    Ve = Ve_j * (amp_mA / -1.0)
    (Vi,Vp),_ = integrate(static, mfn, state0, Ve, pulse_shape, DT, v_rest=V_REST, record="all")
    Vm_n = jnp.where(is_node[None,:], Vi-Vp, -jnp.inf)
    return jnp.max(Vm_n)

# Warmup compile
arr_w = jnp.asarray(make_pulse_array("mono_c", 0.1, int(TSTOP/DT), DT, DELAY))
jax.block_until_ready(run(jnp.float64(-1.0), arr_w))

print(f"MRG D={D} — all 8 pulse shapes after fix")
print(f"{'pulse':>12} {'PW':>6} {'JAX':>10} {'PF':>10} {'err%':>8}")
print("-" * 52)

# Test 2 representative PWs per shape
for pk in PULSES:
    spec = PULSES[pk]
    for pw in [0.10, 0.50]:
        N = int(max(TSTOP, DELAY + (2 if "bi" in pk else 1) * pw + 4.0) / DT)
        tstop_pf = max(TSTOP, DELAY + (2 if "bi" in pk else 1) * pw + 4.0)
        arr = make_pulse_array(pk, pw, N, DT, DELAY)
        try:
            jax_thr = jax_bisect(run, arr[:int(TSTOP/DT)], lo=spec.lo, hi=spec.hi)
            ...
            flag = " <<<" if abs(err) > 5.0 else ""
            print(f"  {pk:>10} {pw:>6.2f} {jax_thr:>10.4f} {pf_thr:>10.4f} {err:>+8.2f}%{flag}")
        except RuntimeError as e:
            print(f"  {pk:>10} {pw:>6.2f}  FAIL: {e}")
