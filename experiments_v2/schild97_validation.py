"""Schild 1997 C-fiber validation suite — v2.

Validates the JAX (Jaxley bwd_euler) implementation of the Schild 1997
C-fiber model against PyFibers/NEURON via intracellular stimulation.

Differs from Schild 1994:
  - naf97mean: m³h fast Na (no l gate, different kinetics, Q10TempA=22)
  - nas97mean: m³h slow Na (different kinetics, Q10TempA=22)
...
Outputs (outputs/schild97_validation/)
---------------------------------------
  data_schild97_traces.json
  fig_schild97_traces.png

Run from project root:
    python experiments_v2/schild97_validation.py
"""
...
from jaxfibers.fibers.schild import (
    build_schild97, node_indices, section_centers_um,
    V_REST,
)
from jaxfibers.stim.intracellular import rectangular_pulse, attach_intra_pulse
from jaxfibers.nrn_baseline import run_intracellular_schild97

from experiments_v2.utils import ensure_dir, save_json

OUT = ensure_dir(ROOT / "outputs" / "schild97_validation")
...
# Jaxley state names for SchildCombined97  (no l_naf gate)
CH = "SchildCombined97"
JAX_STATES = {
    "m_naf97":  f"{CH}_m_naf97",
    "h_naf97":  f"{CH}_h_naf97",
    "m_nas97":  f"{CH}_m_nas97",
    "h_nas97":  f"{CH}_h_nas97",
    "n_kd":     f"{CH}_n_kd",
    "cai":      f"{CH}_cai",
    "cao":      f"{CH}_cao",
}
...
def task_traces():
    ...
    cell, geom = build_schild97(diameter=D, n_nodes=N_NODES, temperature=CELSIUS)
    ...
    t0_pf = time.time()
    res   = run_intracellular_schild97(
    ...
    save_json(data, OUT / "data_schild97_traces.json")
    return data
...
def make_figure(data: dict):
    ...
    lines = [
        ...
        f"",
        f"Note: no l gate (unlike Schild94)",
        f"naf Q10TempA=22 (not 22.85)",
        f"",
    ...
    fig.suptitle(
        f"Schild 1997 C-fiber — JAX (Jaxley bwd_euler) vs PyFibers/NEURON\n"
        f"D={data['diameter']} µm, {N_NODES} compartments, T={CELSIUS}°C",
        fontsize=11,
    )
    path = OUT / "fig_schild97_traces.png"
