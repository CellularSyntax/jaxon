...
* **FEM-derived per-contact lead fields and histology-segmented
  vagus-nerve geometry** produced by the **golgi** platform [golgi1, golgi2]
  — 29 nerves (18 pig, 11 human), built on the Pelot 2020 pig / 2021 human
  SPARC datasets — applied to a multi-contact ring cuff. (A synthetic-nerve
  generator — random uniform fibers in a circle with an eccentric target
  fascicle — and the point-source Ve in `stim/extracellular.py` remain
  available only as demo/testing options.)
...
### 3.1 Anatomy — RESOLVED: histology-segmented geometry + FEM fields (was BLOCKER for Nat Commun)

**Status (RESOLVED / DONE):** the project now runs on **histology-segmented
vagus-nerve morphology and FEM-derived extracellular fields produced by the
golgi platform** [golgi1, golgi2], not on synthetic anatomy.
...
This project now uses **histology-segmented vagus-nerve geometry** — 29
nerves (18 pig, 11 human), built on the Pelot 2020 pig / 2021 human SPARC
datasets — with **FEM-derived per-contact lead fields**, both generated by
golgi and applied over a multi-contact ring cuff. Fields are the FEM
per-contact lead fields, not a point-source approximation; the point-source
Ve in homogeneous tissue (σ = 0.3 S/m) in `stim/extracellular.py` is now
retained only as a demo option.

→ The anatomical-grounding requirement is met: fields and geometry are real
histology-derived FEM output rather than a point-source-in-a-circle
placeholder. This was the biggest single-effort task and the hardest
political requirement to skip; it is **DONE** (see Phase B1).
...
* **B1. ✅ DONE — golgi FEM fields + histology geometry ingested.**
  Extracellular fields are now **FEM-derived per-contact lead fields** and
  nerve geometry is **histology-segmented vagus-nerve morphology**, both
  produced by the golgi platform [golgi1, golgi2] over 29 nerves (18 pig,
  11 human) built on the Pelot 2020 pig / 2021 human SPARC datasets. This
  replaced the point-source Ve in `selectivity_*`. This was the biggest
  single-effort task and the hardest political requirement to skip — now
  resolved (see §3.1).
...
|------|----------|------------|
| Reviewer requires anatomical FEM. | Resolved | golgi FEM fields + histology-segmented geometry now in use (§3.1, B1). |
...
* golgi [golgi1] (software note): Lung D, Jia Y, Moro A, Fachino M,
  Haberbusch M. "golgi: open-source software for automated nerve model
  generation and recruitment simulation." bioRxiv 2026.07.10.737846.
  https://doi.org/10.64898/2026.07.10.737846
* golgi [golgi2] (platform paper): Lung D, Jia Y, Blumer R, Reissig L,
  Zopf LM, Heimel P, Kraus C, Moro A, Fachino M, Haberbusch M. "golgi: an
  open-source graphical platform for image-to-recruitment modeling of
  peripheral nerve stimulation." bioRxiv 2026.07.10.737529.
  https://doi.org/10.64898/2026.07.10.737529
