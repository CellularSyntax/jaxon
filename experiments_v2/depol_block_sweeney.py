"""DC depolarization block — Sweeney A-fiber.
...
--------------------------------------
  ...
  fig_depol_block_sweeney_traces.png
...
PACE_N        = 14      # pulses (several inside the block window)
PACE_PW       = 0.1     # ms
PACE_AMP_NA   = 1.5     # nA

AP_DETECT_LOC = 0.9
# Cathodic (negative) block-field amplitudes: control + deep-block regime where
# JAX and NEURON agree (Sweeney's Na-only block onset differs between solvers, so
# stay in the deep-block plateau >= -8 mA).  14 APs -> full block 9.
...
def main():
    print("=== DC depolarization block — Sweeney A-fiber ===")
    ...
    fig.suptitle(f"DC depolarization block — Sweeney D={DIAMETER} µm", fontsize=11)
    path = OUT / "fig_depol_block_sweeney_traces.png"
