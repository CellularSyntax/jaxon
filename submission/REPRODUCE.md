...
> off-by-one sweep in **`outputs/duke_sweeps/`**, passed to the figure
> scripts via the environment variable **`DUKE_SWEEP_ROOT=outputs/duke_sweeps`**.
> All cohort scripts now honor `DUKE_SWEEP_ROOT` and default to
> `outputs/duke_sweeps`; the earlier uncorrected `outputs/duke_sweeps/`
