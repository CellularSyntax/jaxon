...
\begin{enumerate}
  ...
  \item An end-to-end differentiable selectivity optimisation pipeline
        ...
        optimum, demonstrated on 125 randomised Hussain-style
        multi-fascicle nerve realisations spanning all four
        implemented fibre models (25 seeds per model, with an
        independently randomised target / off-target divider angle
        per seed): rectangular amplitude optimisation achieves
        \textbf{median $\mathrm{SI}=1.000$ on every fibre model},
        with cross-model mean SI of $0.955$ and $70\,\%$ of seeds
        reaching the theoretical maximum $\mathrm{SI}=1.0$
        (Section~\ref{sec:results-selectivity}).
  \item A demonstration of the same gradient-based selectivity
        optimisation pipeline on a cohort of $n=21$ microCT-segmented
        vagus nerve cross-sections (3 swine + 18 human) drawn from
        the SPARC consortium, with FEM-derived per-contact
        extracellular potentials computed by an in-house FEniCSx
        pipeline and the 12-contact MultiContact cuff array
        (Section~\ref{sec:results-selectivity-duke}).  This is, to
        our knowledge, the first cross-species gradient-based
        selectivity optimisation on real microCT-derived
        peripheral nerve anatomies.
\end{enumerate}
...
the anatomical-ceiling cases encountered on the Duke microCT cohort
where the cuff cannot reach the assigned target fascicle at any
amplitude (Section~\ref{sec:discussion}).
