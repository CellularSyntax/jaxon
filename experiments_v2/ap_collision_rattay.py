"""Action potential collision — Rattay C-fiber.

Analogous to ap_collision_sundt.py but for the Rattay HH unmyelinated C-fiber model.
...
collision in the Rattay C-fiber model.

Setup
-----
* Rattay fiber, 4 diameters (0.3, 0.5, 0.8, 1.0 µm), 51 compartments each
...
Outputs (outputs/ap_collision_rattay/)
---------------------------------------
  data_ap_collision_rattay.json
  fig_ap_collision_rattay.png

Run from project root:
    python experiments_v2/ap_collision_rattay.py
"""
...
OUT = ensure_dir(ROOT / "outputs" / "ap_collision_rattay")
...
def _run_one_diameter(D: float) -> dict:
    ...
    fiber = build_rattay_pyfibers(diameter=D, n_nodes=N_NODES, temperature=CELSIUS)
    fiber.record_vm()
...
def make_figure(results: list[dict]) -> None:
    ...
    fig.suptitle(f"AP collision — Rattay C-fiber  ({PULSE_AMP_NA} nA × {PULSE_PW_MS} ms at both ends)",
                 fontsize=10)
    path = OUT / "fig_ap_collision_rattay.png"
...
def main():
    ...
    }, OUT / "data_ap_collision_rattay.json")
    print(f"  -> {OUT / 'data_ap_collision_rattay.json'}")
