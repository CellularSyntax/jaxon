...
the reduced-order fibre sampling standard in GPU-accelerated
selectivity modelling --- reducing each fascicle to one or a few
representative fibres --- systematically overestimates the selectivity
...
number of fibres per target fascicle, and is therefore small in swine
...
simulate the fibres the bias acts on.  Mechanistically the penalty is
a failure to recruit the large human target fascicle from a field
tuned to a few sampled fibres, not off-target spill or reduced drive.
This was measurable only because jaxon simulates the full
${\sim}1000$-fibre population: a fully differentiable, GPU-native
...
The penalty is established \emph{in silico}, at a single fibre diameter
...
$r = +0.72$; fibres-per-fascicle mediates the species gap), not as a
manipulated cause, and we expect the magnitude --- though not the
direction --- to depend on diameter, fibre-type mix, cuff geometry and
the conductivity model.  Confirming the effect across diameters and
fibre populations, and anchoring it experimentally, is the natural next
