...
the cohort analysis is `outputs/duke_sweeps_fixed/` (the corrected off-by-one
sweep), passed to the figure scripts via `DUKE_SWEEP_ROOT=outputs/duke_sweeps_fixed`.
...
| Fig 3 duke | `figures_duke_main.py` | `outputs/duke_sweeps_fixed/` (DUKE_SWEEP_ROOT) | selectivity, deployment penalty, recruitment |

## Named supplementary figures

| Fig | Script | Input data |
|-----|--------|-----------|
| figS_density | `figures_duke_main.py` (secondary output) | `outputs/duke_sweeps_fixed/` |
| figS_khz_block | `figures_phenomena_supp_khz.py` | `outputs/khz_block{,_rattay,_sundt,_sweeney}/data_*.json` |
| figS_optimizer_validation | `fig_optimizer_validation.py` | `outputs/duke_sweeps_fixed/` + `outputs/reviewer_analyses/reanalysis_summary.json` |
...
| figS_* (selectivity/activation, 29) | `figures_supp_activation.py` | `outputs/duke_sweeps_fixed/<nerve>/` |
...
| tab:duke-cohort | `outputs/duke_sweeps_fixed/` (analyze_sparse_sampling.py) |
...
| centroid penalty swine 0.011 / human 0.136; MW p=0.01, U=157, r=0.59 | `outputs/duke_sweeps_fixed/` via analyze_sparse_sampling.py |
| subject-level p=0.011 | `outputs/duke_sweeps_fixed/` (7 human / 11 swine subjects) |
...
- outputs/duke_sweeps_fixed_rs1/rs2/rs3 (58M)  <- reseed robustness variants, not in final paper
