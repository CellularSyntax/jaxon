...
\begin{enumerate}
  ...
  \item \textbf{The pipeline.}  A selectivity-optimization pipeline
        ...
        gradient-based optimization tractable on full fiber populations
        --- not the per-fascicle centroids prior pipelines are limited
        to --- demonstrated on 29 histology-derived vagus anatomies (to
        our knowledge the first cross-species gradient-based selectivity
        optimization on real histology geometry); jaxon further exposes
        exact gradients that NEURON-wrapped pipelines cannot
