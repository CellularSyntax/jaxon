"""Sundt C-fiber validation suite — v2.
...
3. Conduction velocity — all 5 diameters (self-contained, no pre-computed JSON)
4. Figures: traces, SD curves, error analysis (correlation, APE, violin, CV)

Outputs (outputs/sundt_validation/)
-------------------------------------
  data_sundt_traces.json
  data_sundt_sd.json
  data_sundt_cv.json
  fig_sundt_traces.png
  fig_sundt_sd_curves.png
  fig_sundt_analysis.png

Run from project root:
    python experiments_v2/sundt_validation.py
"""
...
OUT = ensure_dir(ROOT / "outputs" / "sundt_validation")
...
# SundtAxon default channel parameters
GNABAR  = 0.04;  GKDRBAR = 0.04;  G_PAS = 1e-4
ENA     = 50.0;  EK      = -90.0; E_PAS = V_REST   # -60 mV
...
DIAMETERS  = [0.3, 0.5, 0.8, 1.0, 1.2]           # µm — standard Sundt range
SD_PWS     = [0.05, 0.1, 0.2, 0.5, 1.0, 2.0]     # ms — longer range for C-fibers
PULSE_KEYS = list(PULSES.keys())

# Trace experiment (Task 1)
TRACE_DIAM   = 0.8   # µm — standard Sundt reference diameter
INTRA_AMP_NA = 0.5
INTRA_PW_MS  = 0.2   # longer than MRG to reliably fire C-fiber
EXTRA_PW_MS  = 0.2
...
def _make_jax_setup(D: float):
    ...
    n0 = float(a_n0 / (a_n0 + b_n0))
    l0 = float(a_l0 / (a_l0 + b_l0))
    state0 = (
...
def task_traces() -> dict:
    ...
    cell_jax, geom_jax = build_sundt(diameter=D, n_nodes=N_NODES)
    ...
    jax_vm_i    = rec_i[0]
    jax_gates_i = {"m": rec_i[1], "h": rec_i[2], "n": rec_i[3], "l": rec_i[4]}

    print("  [intra] NEURON ...", flush=True)
    nr_i = run_intracellular_sundt(
    ...
    @jax.jit
    def _extra_gates_scan(Vi0, Vp0, M0, H0, N0, L0):
        def _step(carry, s):
            Vi, Vp, M, H, N, L = carry
            ve      = Ve_scaled * shape_g[s]
            ve_prev = Ve_scaled * shape_prev_g[s]
            g_eff, i_ion, (M2, H2, N2, L2) = mfn(Vi - Vp, (M, H, N, L), DT)
            Vi2, Vp2 = _be_step(
            ...
            return (Vi2, Vp2, M2, H2, N2, L2), (M2[mid], H2[mid], N2[mid], L2[mid])

        _, (m_t, h_t, n_t, l_t) = jax.lax.scan(
            _step,
            (Vi0, Vp0, M0, H0, N0, L0),
            jnp.arange(N_STEPS),
        )
        return m_t, h_t, n_t, l_t

    M0, H0, N0, L0 = state0
    Vi0_e = jnp.full(n_comp_e, V_REST, dtype=jnp.float64)
    Vp0_e = jnp.zeros(n_comp_e, dtype=jnp.float64)
    m_e, h_e, n_e, l_e = [np.array(x) for x in
                           _extra_gates_scan(Vi0_e, Vp0_e, M0, H0, N0, L0)]
    jax_gates_e = {"m": m_e, "h": h_e, "n": n_e, "l": l_e}

    print(f"  [extra] NEURON (amp={amp_extra:.3f} mA) ...", flush=True)
    nr_e     = run_extracellular_sundt(
    ...
    save_json(data, OUT / "data_sundt_traces.json")
    return data
...
def task_sd_curves() -> dict:
            ...
            for pwi, pw in enumerate(SD_PWS):
                ...
                try:
                    pf_thr = pf_find_threshold(
                        "sundt", D, N_NODES, arr, DT, tstop_pf,
    ...
    save_json(results, OUT / "data_sundt_sd.json")
    return results
...
def task_cv() -> dict:
    """Conduction velocity for all 5 Sundt diameters (self-contained)."""
    print("\n=== Task 3: Conduction velocity ===")
    results: dict = {}
    ...
    save_json(results, OUT / "data_sundt_cv.json")
    return results
...
def fig_traces(data: dict) -> None:
    ...
    ax = axs[0, 1]
    gate_colors = {"m": "C0", "h": "C2", "n": "C4", "l": "C6"}
    ...
    ax.set_title("(B) Intracellular gates  (m, h: Na;  n, l: K)")
    ...
    fig.suptitle(f"Sundt D={data['diameter']} µm — PyFibers/NEURON vs JAX", fontsize=11)
    path = OUT / "fig_sundt_traces.png"
...
def fig_sd_curves(sd: dict) -> None:
    ...
    fig.suptitle("Sundt SD curves — PyFibers solid, JAX dashed (all 5 diameters)", fontsize=11)
    path = OUT / "fig_sundt_sd_curves.png"
    fig.savefig(path, dpi=150, bbox_inches="tight")
    print(f"  → {path}")
...
def fig_analysis(sd: dict, cv: dict) -> None:
    ...
    fig.suptitle(
        "Sundt validation analysis — JAX coupled solver vs PyFibers/NEURON\n"
        f"N={N_NODES} nodes, 1 mm point source, σ={SIGMA} S/m, T={CELSIUS}°C",
        fontsize=11,
    )
    path = OUT / "fig_sundt_analysis.png"
    fig.savefig(path, dpi=150, bbox_inches="tight")
    print(f"  → {path}")
